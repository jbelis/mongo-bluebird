...
var _instance = function (settings) {
    var getURI = function (settings) {
        var uri = ["mongodb://"];
        if (settings.username && settings.password) {
            uri.push(settings.username);
            uri.push(":");
            uri.push(settings.password);
            uri.push("@");
        }
        uri.push(settings.host);
        uri.push(":");
        uri.push(settings.port || 27017);
        uri.push("/");
        uri.push(settings.db);

        var option = {
            "server": {}
        };
        if (settings.ssl) {
            option.server.ssl = true;
        }
        if (settings.poolSize) {
            option.server.poolSize = settings.poolSize;
        }
        return {
            uri: uri.join(""),
            option: option
    ...
    this.connectOption = getURI(settings);
    this.collections = {};
    this.promise = new Promise(function (resolve, reject) {
        ...
        //    }
        //});
        MongoClient.connect(this.connectOption.uri, this.connectOption.option, function (err, db) {
            if (err) {
                reject(err);
            } else {
                resolve(db);
